...
export default function LandingPage() {
  ...
  function toggle(id: string) { setAddons(a => a.includes(id) ? a.filter(x => x !== id) : [...a, id]); }
  ...
  async function checkout() {
    const res = await fetch("/api/checkout", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ items: ["core", ...addons], email }) });
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="max-w-4xl mx-auto px-6 py-20 text-center">
        ...
        <a href="#pricing" className="bg-blue-600 text-white px-8 py-3 rounded-lg text-lg font-medium">See Pricing</a>
      </div>
      ...
      <div className="bg-gray-50 py-16">
        <div className="max-w-4xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            ...
            ].map(f => <div key={f.t} className="bg-white rounded-lg p-6 shadow-sm"><h3 className="font-bold mb-2">{f.t}</h3><p className="text-gray-600 text-sm">{f.d}</p></div>)}
          </div>
        </div>
      </div>
      ...
      <div id="pricing" className="py-16">
        <div className="max-w-2xl mx-auto px-6">
          ...
          <div className="bg-blue-50 border-2 border-blue-200 rounded-xl p-6 mb-6">
            <div className="flex justify-between items-start"><div><h3 className="text-xl font-bold">Core System</h3><p className="text-gray-600 text-sm mt-1">Widget + Dashboard + Tables + Email + Walk-ins</p></div><div className="text-3xl font-bold">$299</div></div>
          ...
          <div className="space-y-3 mb-8">
            {addonList.map(a => (
              <label key={a.id} className={`flex items-center justify-between p-4 rounded-lg border-2 cursor-pointer transition ${addons.includes(a.id) ? "border-blue-500 bg-blue-50" : "border-gray-200 hover:border-gray-300"}`}>
                <div className="flex items-center gap-3">
                  <input type="checkbox" checked={addons.includes(a.id)} onChange={() => toggle(a.id)} className="w-4 h-4" />
                  <div><div className="font-medium">{a.name}</div><div className="text-sm text-gray-500">{a.desc}</div></div>
                </div>
          ...
          <div className="bg-gray-50 rounded-xl p-6">
            ...
            <button onClick={checkout} disabled={!email} className="w-full bg-blue-600 text-white py-3 rounded-lg text-lg font-medium disabled:opacity-50">Purchase Now</button>
        ...
        <div className="max-w-3xl mx-auto px-6">
          ...
          <div className="bg-white rounded-xl shadow overflow-hidden">
            <table className="w-full text-sm">
              <thead><tr className="border-b bg-gray-50"><th className="text-left p-4"></th><th className="p-4 text-center font-bold text-blue-600">ReserveKit</th><th className="p-4 text-center">OpenTable</th><th className="p-4 text-center">Resy</th></tr></thead>
              <tbody>
                {[["Upfront cost", "$299 once", "$0", "$0"], ["Monthly fee", "$0–$15", "$249–$449", "$249–$899"], ["Per-cover fee", "Never", "$0.25–$1.00", "None"], ["Year 1 cost", "$299–$479", "$3,888+", "$2,988+"], ["Own your data", "✓", "✗", "✗"], ["Self-hostable", "✓", "✗", "✗"]].map(([l, ...v]) => (
                  <tr key={l} className="border-b last:border-0"><td className="p-4 font-medium">{l}</td>{v.map((c, i) => <td key={i} className={`p-4 text-center ${i === 0 ? "font-medium text-blue-600" : "text-gray-600"}`}>{c}</td>)}</tr>
                ))}
              </tbody>
            </table>
          </div>
